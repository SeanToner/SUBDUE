...
int main (int argc, char **argv)
{
   ...
   FILE *fp;
   fp = fopen("bestSubs.txt", "ab+");
   fclose(fp);

   FILE *fp1;
   fp1 = fopen("bestSubs.txt", "w");
   fclose(fp1);

   while(i < noOfGraphs)
   {
	 parameters = GetParameters(argc, argv, i);
	 noOfGraphs = parameters -> noGraphs;
...
Parameters *GetParameters(int argc, char *argv[], int graphNo)
{
   Parameters *parameters;
   int i;
   double doubleArg;
   ...
   while (i < (argc - 1))
   {
	  ...
	  else if (strcmp(argv[i], "-noGraphs") == 0)
      {
         i++;
         sscanf(argv[i], "%lu", &ulongArg);
         if (ulongArg == 0)
         {
            fprintf(stderr, "%s: No of graphs must be greater than zero\n", argv[0]);
            exit(1);
         }
         parameters->noGraphs = ulongArg;
		 printf("%lu", ulongArg);
      }
   ...
   // read graphs from input file
	...
	if(parameters->noGraphs > 1)
	{
		char integer_string[512];
		sprintf(integer_string, "%d", graphNo);

		strcat(parameters->inputFileName, integer_string); 
	}
