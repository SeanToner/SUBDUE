...
BOOLEAN GraphMatch2(Graph *g1, Instance *g2, LabelList *labelList,
                   double threshold, double *matchCost, VertexMap *mapping)
{
   double cost;
   ...
   // call InexactGraphMatch with larger graph first
   if (g1->numVertices < g2->numVertices)
      cost = InexactGraphMatch2(g2, g1, labelList, threshold, mapping);
   else 
      cost = InexactGraphMatch3(g1, g2, labelList, threshold, mapping);
...
double InexactGraphMatch2(Instance *g1, Graph *g2, LabelList *labelList,
                         double threshold, VertexMap *mapping)
{
   ULONG i, v1, v2;
   ULONG nv1 = g1->numVertices;
   ...
   if (orderedVertices == NULL)
      OutOfMemoryError("orderedVertices");
   OrderVerticesByDegree2(g1, orderedVertices);
      ...
      if (node.cost < bestNode.cost) 
      {
         ...
         { // expand node's partial mapping
           // compute mappings between graphs' vertices
            ...
            if ((newCost <= threshold) && (newCost < bestNode.cost)) 
            {
               for (i = 0; i < g1->verticesArray[v1].numEdges; i++) 
               {
                  edge = & g1->edgesArray[g1->verticesArray[v1].edges[i]];
                  if (v1 == edge->vertex1)
                     v2 = edge->vertex2;
                  else 
                     v2 = edge->vertex1;
            ...
            for (v2 = 0; v2 < nv2; v2++) 
            {
               if (mapped2[v2] == VERTEX_UNMAPPED) 
               {
                  ...
                  newCost = node.cost;
                  newCost += SUBSTITUTE_VERTEX_LABEL_COST *
                     LabelMatchFactor(g1->verticesArray[v1].label,
                                      g2->vertices[v2].label, labelList);
                  if ((newCost <= threshold) && (newCost < bestNode.cost)) 
                  {
                     cost = DeletedEdgesCost2(g1,g2,v1,v2,mapped1,labelList);
                     newCost += cost;
                     cost = InsertedEdgesCost(g2, v2, mapped2);
                     newCost += cost;
                  }
...
double InexactGraphMatch3(Graph *g1, Instance *g2, LabelList *labelList,
                         double threshold, VertexMap *mapping)
{
   ULONG i, v1, v2;
   ULONG nv1 = g1->numVertices;
         ...
         { // expand node's partial mapping
           // compute mappings between graphs' vertices
            ...
            if ((newCost <= threshold) && (newCost < bestNode.cost)) 
            {
               if (node.depth == (nv1 - 1)) 
               {
                  cost = InsertedVerticesCost2(g2, mapped2);
                  newCost += cost;
               }
            }
            if ((newCost <= threshold) && (newCost < bestNode.cost)) 
            ...
            for (v2 = 0; v2 < nv2; v2++) 
            {
               if (mapped2[v2] == VERTEX_UNMAPPED) 
               {
                  ...
                  newCost += SUBSTITUTE_VERTEX_LABEL_COST *
                     LabelMatchFactor(g1->vertices[v1].label,
                                      g2->verticesArray[v2].label, labelList);
                  if ((newCost <= threshold) && (newCost < bestNode.cost)) 
                  {
                     cost = DeletedEdgesCost2(g2,g1,v1,v2,mapped1,labelList);
                     newCost += cost;
                     cost = InsertedEdgesCost2(g2, v2, mapped2);
                     newCost += cost;
                  }
                  // if complete mapping, add cost for any unmapped vertices
                  // and edges in g2
...
void OrderVerticesByDegree2(Instance *g, ULONG *orderedVertices)
{
   ULONG nv = g->numVertices;
   ULONG i, j;
   ULONG degree;
   ...
   for (i = 0; i < nv; i++) 
   {
      degree = g->verticesArray[i].numEdges;
      j = i;
      while ((j > 0) && (degree > vertexDegree[j-1])) 
      {
         vertexDegree[j] = vertexDegree[j-1];
...
double DeletedEdgesCost2(Instance *g1, Graph *g2, ULONG v1, ULONG v2,
                        ULONG *mapped1, LabelList *labelList)
{
   ULONG e1, e2;
   Edge *edge1, *edge2;
   ...
   // try to match each edge involving v1 to an edge involving v2
   for (e1 = 0; e1 < g1->verticesArray[v1].numEdges; e1++) 
   {
      edge1 = & g1->edgesArray[g1->verticesArray[v1].edges[e1]];
      if (edge1->vertex1 == v1)
         otherVertex1 = edge1->vertex2;
      else 
         otherVertex1 = edge1->vertex1;
...
double InsertedEdgesCost2(Instance *g2, ULONG v2, ULONG *mapped2)
{
   ULONG e2;
   Edge *edge2;
   double totalCost = 0.0;

   for (e2 = 0; e2 < g2->verticesArray[v2].numEdges; e2++) 
   {
      edge2 = & g2->edgesArray[g2->verticesArray[v2].edges[e2]];
      if ((! edge2->used) &&
          (mapped2[edge2->vertex1] != VERTEX_UNMAPPED) &&
          (mapped2[edge2->vertex2] != VERTEX_UNMAPPED))
         totalCost += INSERT_EDGE_COST;
...
double InsertedVerticesCost2(Instance *g2, ULONG *mapped2)
{
   ULONG v;
   ULONG v2;
   ULONG e;
   ...
   for (v = 0; v < g2->numVertices; v++)
      if (mapped2[v] == VERTEX_UNMAPPED) 
      {
         cost += INSERT_VERTEX_COST;
         // add cost for edges to mapped vertices and self edges
         for (e = 0; e < g2->verticesArray[v].numEdges; e++) 
         {
            if (v == g2->edgesArray[g2->verticesArray[v].edges[e]].vertex1)
               v2 = g2->edgesArray[g2->verticesArray[v].edges[e]].vertex2;
            else 
               v2 = g2->edgesArray[g2->verticesArray[v].edges[e]].vertex1;
            if ((mapped2[v2] != VERTEX_UNMAPPED) || (v2 == v))
               cost += INSERT_EDGE_WITH_VERTEX_COST;
         }
      }
