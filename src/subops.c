...
Instance *AllocateInstance(ULONG v, ULONG e)
{
   ...
   instance->newEdge = 0;
   instance->verticesArray = NULL;
   instance->edgesArray = NULL;
   ...
   if (v > 0) 
   {
	  ...
	  instance->verticesArray = (Vertex *) malloc(sizeof(ULONG) * v);
   ...
   if (e > 0) 
   {
	  ...
	  instance->edgesArray = (Edge *) malloc(sizeof(ULONG) * e);
...
void InstanceListInsert2(Instance *instance, InstanceList *instanceList,
                        BOOLEAN unique, BOOLEAN allowOverlap)
{
   InstanceListNode *instanceListNode;

   if ((! unique) ||
       (unique && (!allowOverlap ||  ! MemberOfInstanceList(instance, instanceList)))) 
   {
      instanceListNode = AllocateInstanceListNode(instance);
      instanceListNode->next = instanceList->head;
      instanceList->head = instanceListNode;
...
{
   ULONG i;
   ULONG v = instance1->numVertices;
   ULONG e = instance1 ->numEdges;

   // check that instances have same number of vertices and edges
   if ((v != instance2->numVertices) ||
...
BOOLEAN NewEdgeMatch2(Graph *g1, Instance *inst1, Instance *g2,
                     Instance *inst2, Parameters *parameters, 
                     double threshold, double *cost)
{
   VertexMap *subMapping;
   ...
   //    - are the new vertex labels (if new vertex added) the same?
   //
   if ((g1->edges[newEdge1].label == g2->edgesArray[newEdge2].label) &&
       (g1->edges[newEdge1].directed == g2->edgesArray[newEdge2].directed) &&
	   (((newVertex1 == VERTEX_UNMAPPED) && (newVertex2 == VERTEX_UNMAPPED)) ||
		((newVertex1 != VERTEX_UNMAPPED) && (newVertex2 != VERTEX_UNMAPPED) &&
		 (g1->vertices[newVertex1].label == g2->verticesArray[newVertex2].label))))
   {
      //
      // First, map the second instance to the parent of the first instance,
      // and then see if their mappings match.
      ...
      // 
      if (((g1->edges[newEdge1].vertex1 == 
            g2->edgesArray[newEdge2].vertex1) &&
           (g1->edges[newEdge1].vertex2 == 
            g2->edgesArray[newEdge2].vertex2) &&
           (inst1Vertex1 == inst2Vertex1) &&
           (inst1Vertex2 == inst2Vertex2)) ||
          ((!g2->edgesArray[newEdge2].directed) &&
           (g1->edges[newEdge1].vertex1 == 
            g2->edgesArray[newEdge2].vertex2) &&
           (g1->edges[newEdge1].vertex2 == 
            g2->edgesArray[newEdge2].vertex1) &&
   ...
   if (GraphMatch2(g1, g2, labelList, threshold, NULL, subMapping))
   {
      // Declare some variables that were not needed until now
      ULONG value;
      ULONG index = 0;
