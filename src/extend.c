...
SubList *ExtendSub(Substructure *sub, Parameters *parameters)
{
   ...
   newInstanceList = ExtendInstances(sub->instances, posGraph, parameters ->allowInstanceOverlap);
   negInstanceList = NULL;

   if (negGraph != NULL)
      negInstanceList = ExtendInstances(sub->negInstances, negGraph, parameters ->allowInstanceOverlap);
...
InstanceList *ExtendInstances(InstanceList *instanceList, Graph *graph, BOOLEAN allowOverlap)
{
   InstanceList *newInstanceList;
   ...
   while (instanceListNode != NULL) 
   {
      ...
      for (v = 0; v < instance->numVertices; v++) 
      {
		 ...
		 graph->vertices[instance->vertices[v]].TimesAddedToInstanceList = 
			 graph->vertices[instance->vertices[v]].TimesAddedToInstanceList + 1;

	if(allowOverlap || graph->vertices[instance->vertices[v]].TimesAddedToInstanceList < 2)
	{
         for (e = 0; e < vertex->numEdges; e++) 
         {
            ...
            if (! edge->used) 
            {
               ...
               InstanceListInsert2(newInstance, newInstanceList, TRUE, allowOverlap);
            }
         }
		 }
	  }
   ...
   InstanceListNode *instanceListNode1 = instanceList->head;
   Instance *instance2;
   while (instanceListNode1 != NULL) 
   {
      instance2 = instanceListNode1->instance;


   int j=0;

    for (j = 0; j < instance2->numVertices; j++) 
      {
		
          graph->vertices[instance2->vertices[j]].TimesAddedToInstanceList = 0;
	}
                                 ...
                                 Graph *graph)
{
   ...
   Edge *newEdgeList = newEdgeList = (Edge *) realloc(newInstance->edgesArray, (sizeof(Edge) * (newInstance->numEdges)));
   newInstance->edgesArray =  newEdgeList;

   Vertex *newVertexList = newVertexList = (Vertex *) realloc(newInstance->verticesArray, (sizeof(Vertex) * (newInstance->numVertices)));
   newInstance->verticesArray =  newVertexList;

   // convert vertices
   for (i2 = 0; i2 < newInstance->numVertices; i2++) 
   {
      vertex2 = & graph->vertices[newInstance->vertices[i2]];

      newInstance->verticesArray[i2].label = vertex2->label;
      newInstance->verticesArray[i2].numEdges = 0;
      newInstance->verticesArray[i2].edges = NULL;
      newInstance->verticesArray[i2].used = FALSE;
   }

   // convert edges
   for (i2 = 0; i2 < newInstance->numEdges; i2++) 
   {
      edge2 = & graph->edges[newInstance->edges[i2]];
      ...
      while ((! found1) || (! found2)) 
      {
         if (newInstance->vertices[j2] == edge2->vertex1) 
         {
            v12 = j2;
            found1 = TRUE;
         }
         if (newInstance->vertices[j2] == edge2->vertex2) 
         {
            v22 = j2;
            found2 = TRUE;
         }
      ...
      newInstance->edgesArray[i2].vertex1 = v12;
      newInstance->edgesArray[i2].vertex2 = v22;
      newInstance->edgesArray[i2].label = edge2->label;
      newInstance->edgesArray[i2].directed = edge2->directed;
      newInstance->edgesArray[i2].used = FALSE;


      // add edge to appropriate vertices
      vertex2 = & newInstance->verticesArray[v12];
      vertex2->numEdges++;
      vertex2->edges =
         (ULONG *) realloc(vertex2->edges, sizeof(ULONG) * vertex2->numEdges);
 
      vertex2->edges[vertex2->numEdges - 1] = i2;
      if (v12 != v22) 
      {
         vertex2 = & newInstance->verticesArray[v22];
         vertex2->numEdges++;
         vertex2->edges =
            (ULONG *) realloc(vertex2->edges, sizeof(ULONG) * vertex2->numEdges);
         ...
         if (instanceListNode->instance != NULL) 
         {
            instance = instanceListNode->instance;
               ...
               if (threshold == 0.0)
               {
                  ...
                  {
                     if (NewEdgeMatch2 (sub->definition, subInstance, instance, 
               ...
               {
                  if (GraphMatch2(sub->definition, instance, labelList,
